...
> TypeScript code review guide covering type system, generics, conditional types, strict mode, async/await patterns, and core topics.
...
// ✅ Use unknown + type guard for unknown types
...
```typescript
// ❌ Generic lacks constraint, cannot access property
...
```typescript
// ✅ Provide reasonable default type
...
// Generic argument can be omitted
...
```typescript
// ✅ Transform all properties of object type
type Nullable<T> = {
...
```typescript
// ❌ Promise.all fails fast on single error
async function fetchAllUsers(ids: string[]) {
  const users = await Promise.all(ids.map(fetchUser));
  return users;  // Fails completely if any request fails
}

// ✅ Promise.allSettled retrieves all results
async function fetchAllUsers(ids: string[]) {
...
```typescript
// ❌ Race condition: Old request may overwrite newer request
function useSearch() {
  ...
  useEffect(() => {
    fetch(`/api/search?q=${query}`)
      .then(r => r.json())
      .then(setResults);  // Stale response may resolve later!
  }, [query]);
}
...
```typescript
// ❌ Mutable parameter may be modified unexpectedly
...
### Type System
- [ ] Avoid using any (use unknown + type guards instead)
- [ ] Complete interface and type definitions with meaningful names
...
- [ ] Avoid over-engineering generics (KISS principle)

### Strict Mode
- [ ] tsconfig.json enables strict: true
- [ ] Enabled noUncheckedIndexedAccess
- [ ] Avoid @ts-ignore (use @ts-expect-error instead)

### Async Code
- [ ] async functions include error handling
...
- [ ] Race conditions handled using AbortController

### Immutability
- [ ] Do not directly mutate function parameters
